...
def load_image_folder(folder_name, batch_size, truncate_number=None, val_frac=0.2, test_frac=0.1, num_workers=0):
    """
    ...
    if not 0 < val_frac + test_frac < 1:
        raise ValueError(f"Invalid val_frac + test_frac. Expected 0-1. Got {val_frac + test_frac}")
    ...
    training_loader = DataLoader(training_set, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=num_workers, worker_init_fn=seed_worker, generator=torch.Generator().manual_seed(42))
    validation_loader = DataLoader(validation_set, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=num_workers // 4, worker_init_fn=seed_worker, generator=torch.Generator().manual_seed(42))
    test_loader = DataLoader(test_set, batch_size=batch_size, shuffle=False, drop_last=True, num_workers=num_workers // 4, worker_init_fn=seed_worker, generator=torch.Generator().manual_seed(42))
