...
class ResidualBlockTranspose(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride=1, output_padding=0):
        if stride != 3:
            warnings.warn(f"A stride of {stride} used with a transpose convolution with kernel size 3 will produce checkerboard patterns. These can be overcome with more training, but may be detrimental")
        super().__init__()
...
class ResidualBlockResize(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, factor=1, interpolation=transforms.InterpolationMode.BILINEAR):
        super().__init__()
        self.factor = factor
    ...
    def forward(self, X):
        Y = F.relu(self.bn1(self.conv1(X)))
        Y = transforms.functional.resize(Y, [int(s * self.factor) for s in Y.shape[-2:]], interpolation=self.interpolation)
        Y = self.bn2(self.conv2(Y))
        if self.conv3:
            X = self.conv3(X)
            X = transforms.functional.resize(X, [int(s * self.factor) for s in X.shape[-2:]], interpolation=self.interpolation)
        Y += X
        return F.relu(Y)
